     ...
     <div className="flex gap-4 justify-between my-4">
        ...
        <div className="overflow-y-scroll h-[500px] w-[100%] p-4 border-2">
          ...
          {users?.length > 0 ? users?.map((item, key) => {
            return <div key={key} className="flex mb-2 mt-8 justify-between p-4 border-2 shadow-md rounded-md"><Link href={'/admin/products'}>{item.email}</Link> <Trash2 className="cursor-pointer" color='grey' size={20} /> </div>
        ...
        <div className="overflow-y-scroll h-[500px] w-[100%] p-4 ">
          ...
          {products?.length > 0 ? products?.map((item, key) => {
            return <div key={key} className="flex mb-2 mt-8 justify-between p-4 border-2 shadow-md rounded-md"><Link href={'/admin/products'}>{item.name}</Link> <Trash2 className="cursor-pointer" color='grey' size={20} /> </div>
