...
import { addProduct, getProductById } from '@/app/actions/addProduct/addProduct'
...
function Page() {
    ...
    const [populate, setPopulate] = useState<any>({})
    ...
    useEffect(()=>{
        async function fetchProduct(){
            const res = await getProductById(path.split('/').pop())
            console.log(res)
            setPopulate(res)
        }
        ...
        <div className="max-w-5xl m-auto p-4 h-full">
            ...
            <form className='w-[60%] m-auto border-2 p-8 rounded-md flex flex-col gap-4' >
                <div className='flex flex-col '>
                    ...
                    <input value={populate.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className='border-2 rounded-md p-2 max-w-[200px]' id='name' type="text" name="" placeholder='name' required />
                </div>
                <div className='flex flex-col'>
                    <label id='description'>Description: </label>
                    <textarea value={populate.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className='border-2 rounded-md p-2 max-w-[80%]' placeholder='description' typeof='text' id='description' />
                </div>
                <div className='flex flex-col'>
                    <label className=''>License Price: </label>
                    <div>
                        <input value={populate.licensePrice} onChange={(e) => setForm({ ...form, licensePrice: parseFloat(e.target.value) })} className='border-2 mr-2 rounded-md p-2 max-w-[100px]' type='number' required />$
                    </div>
                </div>
                <hr />
                <div className='flex flex-col'>
