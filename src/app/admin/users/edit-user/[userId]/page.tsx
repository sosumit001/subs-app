...
function Page() {
    ...
    useEffect(() => {
        ...
        async function fetchUser(){
            const res = await getUserById(path.split('/').pop())
            console.log(res)
        }
        ...
        <div className="max-w-5xl m-auto p-4 h-full">
            ...
            <form className='w-[50%] m-auto border-2 p-8 rounded-md flex flex-col justify-center items-center gap-4' >
                <div className='flex flex-col w-[80%] '>
                    ...
                    <input type='email' value={populate.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className='border-2 rounded-md p-2 ' id='email' name="email" placeholder='email' required />
                </div>
                ...
                <button onClick={(e) => handleSubmit(e)} className='flex items-center px-4 py-2 bg-black text-white rounded'>Update User {loader && <Loader size={15} color='white' />}</button>
